...
static void test_parse_array() {
    ...
    for (i = 0; i < 4; ++i) {
        lept_value* cur = lept_get_array_element(&v, i);
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(cur));
        EXPECT_EQ_SIZE_T(i, lept_get_type(cur));
        for (j = 0; j < i; ++j) {
            lept_value* curChild = lept_get_array_element(cur, j);
            EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(curChild));
            EXPECT_EQ_DOUBLE((double)j, lept_get_number(curChild));
        }
    }
    lept_free(&v);
}
